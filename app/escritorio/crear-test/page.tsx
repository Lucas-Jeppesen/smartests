              ...
              {(field) => (
                <>
                  <div className="flex justify-between mb-2">
                    ...
                    <ModalTriggerButton
                      type="create-asig"
                      className='bg-yellow-1 hover:bg-green-1 hover:text-yellow-1 text-green-2 font-medium text-xs px-4 py-0.5 rounded cursor-pointer border border-green-1 transition-all duration-300 ease-in-out'
                    >
                      Crear nueva
                    </ModalTriggerButton>
                  </div>
        ...
        >
          {([canSubmit, isSubmitting]) => (
            <button
              type="submit"
              disabled={!canSubmit}
              className="px-4 py-2 bg-green-4 hover:bg-green-2 text-white rounded duration-200"
            >
              {isSubmitting ? "Subiendo pdf..." : "Generar"}
            </button>
          )}
        </form.Subscribe>
      </form>
      </div>
      {user && <CreateAsigModal userId={user.id} />}
    </ModalProvider>
  );
}
